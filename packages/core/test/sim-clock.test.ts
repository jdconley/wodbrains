...
describe('sim clock', () => {
  it('accumulates dt into fixed ticks', () => {
    const result = advanceFixedStep(
      { simNowMs: 0, accumulatorMs: 0 },
      250,
      0,
      { tickMs: 100, maxCatchupTicks: 10, maxCorrectionPerTickMs: 0 },
    );
  ...
  it('respects maxCatchupTicks and keeps backlog', () => {
    const result = advanceFixedStep(
      ...
      1000,
      0,
      { tickMs: 100, maxCatchupTicks: 3, maxCorrectionPerTickMs: 0 },
    );
  ...
  it('applies bounded drift correction per tick', () => {
    const result = advanceFixedStep(
      ...
      100,
      500,
      { tickMs: 100, maxCatchupTicks: 10, maxCorrectionPerTickMs: 20 },
    );
  ...
  it('smooths monotonic offset updates', () => {
    ...
    const smoothed = updateMonotonicOffset(initial, {
